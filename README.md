...
The whole French upper-secondary programme is written — Grade 10, Grade 11 and Grade 12, mathematics and physics-chemistry: a hundred and thirteen destinations, a hundred and thirty-one narrated lessons with twenty-two kinds of interactive tool, two hundred and sixty-four exercises with typed answers, and seventeen routes flown in the order the notions build on one another. The [initial implementation target](#initial-implementation-target) below is implemented as a complete vertical slice: bilingual local onboarding, the three worlds with every region visible from the first session, the derivative tool at four depths (Grade 11 to the second preparatory year), eight linked phenomena, the historical mission *Galileo in Padua: the inclined plane*, three reusable simulation engines, five exercise types with evidence collection, the virtual backpack (application coverage and mastery), learner and guide views, offline use as an installable progressive web app, and local persistence with export/import.
