...
<div
  ...
  style="padding: calc(var(--space-6) + var(--safe-top)) 0 var(--space-7)"
>
    ...
    <ul class="list">
      {#each profile.summaries as s (s.id)}
        <li class="card cluster" style="justify-content: space-between">
          ...
          <div class="cluster">
            <button class="btn btn--sm btn--primary" type="button" onclick={() => open(s.id)}
              >{t('profiles.open')}</button
            >
            <button
              class="btn btn--sm btn--danger"
              type="button"
              onclick={() => remove(s.id, s.name)}>{t('profiles.delete')}</button
            >
          </div>
        </li>
      {/each}
  ...
  <div class="cluster">
    ...
    {#if profile.active}<a class="btn btn--ghost" href="{base}/universe">{t('returning.map')}</a
      >{/if}
  </div>
</div>
