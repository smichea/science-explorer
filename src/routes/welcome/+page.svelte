<script lang="ts">
  ...
  async function submit(event: SubmitEvent) {
    ...
    await profile.create(
      { name, age: Number(ageText), locale: locale.current },
      config,
      content.pkg.manifest.version
    );
    confirmed = true;
  }
...
<div class="welcome">
  <div class="container container--narrow stack">
    ...
    {#if confirmed && profile.active && config}
      ...
      <section
        class="card card--paper stack"
        aria-labelledby="horizon-title"
        data-testid="horizon-confirmation"
      >
        <h1 id="horizon-title">{t('horizon.title')}</h1>
        <p>
          {t('horizon.intro', {
            name: profile.active.name,
            age: profile.active.age,
            stage: stageTitle(profile.active.inferredStage),
            years: horizon?.horizonYears ?? 3,
          })}
        </p>
        <ol class="route" aria-label={t('horizon.title')}>
          {#each horizon?.stages ?? [] as stage, i (stage)}
            <li class="route__stage">
              <span class="route__when"
                >{i === 0 ? t('horizon.now') : t('horizon.plusYears', { n: i })}</span
              >
        ...
        {#if horizon?.targets.length}
          <p class="muted">
            {t('horizon.targets', { targets: horizon.targets.map((x) => L(x.title)).join(' · ') })}
          </p>
        {/if}
        ...
        <div class="cluster">
          <a class="btn btn--primary" href="{base}/universe" data-testid="open-map"
            >{t('horizon.confirm')}</a
          >
        </div>
      </section>
    {:else if returning && profile.active && config}
      ...
      <section class="card card--paper stack" aria-labelledby="returning-title">
        ...
        <p>
          {t('returning.summary', {
            age: profile.active.age,
            path: (horizon?.stages ?? []).map(stageTitle).join(' → '),
          })}
        </p>
            ...
            {#if editingAge}
              <div class="cluster">
                <input
                  ...
                  max="120"
                  bind:value={newAge}
                  aria-label={t('welcome.age')}
                />
                <button class="btn" type="button" onclick={updateAge}>{t('common.save')}</button>
              </div>
            {:else}
              <div class="cluster">
                <button class="btn btn--sm" type="button" onclick={() => profile.confirmAge()}
                  >{t('returning.confirmAgeYes')}</button
                >
                <button
                  ...
                  onclick={() => {
                    editingAge = true;
                    newAge = String(profile.active?.age ?? '');
                  }}>{t('returning.confirmAgeUpdate')}</button
                >
              </div>
            {/if}
          </div>
    ...
    {:else}
      <section class="card card--paper stack" aria-labelledby="welcome-title">
        ...
        <form class="stack" onsubmit={submit} novalidate>
          ...
          <button class="btn btn--primary" type="submit" data-testid="enter-universe"
            >{t('welcome.enter')}</button
          >
              ...
              {#each profile.summaries as s (s.id)}
                <li>
                  <button
                    ...
                    type="button"
                    onclick={async () => {
                      await profile.open(s.id);
                      await learning.bind(s.id);
