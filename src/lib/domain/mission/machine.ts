...
export function stepsForVariant(mission: MissionDefinition, variantId: string): StepDefinition[] {
  const variant =
    mission.learning.depthVariants.find((v) => v.id === variantId) ??
    mission.learning.depthVariants[0];
...
function statusForStep(step: StepDefinition | undefined): MissionSessionStatus {
  ...
  if (
    step.completion.kind === 'exercises' ||
    step.completion.kind === 'explanation' ||
    step.completion.kind === 'choice' ||
    step.completion.kind === 'inputs'
  )
...
export function createSession(
  ...
  return {
    session,
    evidence: stepEvidence(mission, steps[0], session, ctx, 'enter'),
...
function stepEvidence(
  ...
  const autonomy = autonomyFromHints(
    state?.hintsOpened.length ?? 0,
    state?.guideHints === 'revealed'
  );
  ...
  for (const ev of step.evidence) {
    ...
    // Live events are emitted as they happen (one per measurement, attempt or hint), never summarised again.
    if (when === 'complete' && LIVE.has(ev.type)) continue;
    ...
    if (ev.type === 'transfer_completed' || ev.type === 'tool_selected_for_model')
      draft.result = state?.toolCorrect === false ? 'incorrect' : 'correct';
    if (ev.type === 'prediction_recorded')
      draft.payload = { choice: state?.choice, inputs: state?.inputs };
    if (ev.type === 'measurement_recorded')
      draft.payload = { measurements: state?.measurements ?? [] };
    drafts.push(draft);
  }
...
function isComplete(step: StepDefinition, state: StepState): boolean {
  ...
  switch (c.kind) {
    ...
    case 'choice':
      return !!state.choice;
    case 'inputs':
      return c.inputs.every(
        (i) => state.inputs?.[i.id] !== undefined && state.inputs?.[i.id] !== ''
      );
    case 'simulation':
      return (
        (state.measurements?.length ?? 0) >= c.minMeasurements &&
        (state.parameterChanges ?? 0) >= c.minParameterChanges
      );
    case 'exercises':
      return c.exerciseIds.every(
        (id) => state.answers?.[id]?.correct || (state.answers?.[id]?.attempts ?? 0) >= 3
      );
    case 'explanation':
...
export function reduce(
  ...
  const next: MissionSession = {
    ...session,
    stepStates: { ...session.stepStates },
    guideCommands: [...session.guideCommands],
  ...
  const autonomy = () =>
    autonomyFromHints(state.hintsOpened.length, state.guideHints === 'revealed');
  const base = {
    missionId: mission.id,
    sessionId: session.id,
    stepId: step.id,
  ...
  const goTo = (stepId: string | null) => {
    ...
    if (targetState.status === 'pending' || targetState.status === 'skipped')
  ...
  switch (command.type) {
        ...
        if (correct !== undefined)
          evidence.push(
            makeEvidence(
              {
                ...base,
                type: correct ? 'exercise_solved' : 'exercise_attempted',
                result: correct ? 'correct' : 'incorrect',
                autonomy: autonomy(),
                discriminator: `choice:${command.choiceId}`,
              },
              ctx
            )
          );
    ...
    case 'measurement':
      state.measurements = [
        ...(state.measurements ?? []),
        { t: command.t, value: command.value, label: command.label },
      ];
      evidence.push(
        makeEvidence(
          {
            ...
            nodeId: step.evidence.find((e) => e.type === 'measurement_recorded')?.nodeId,
            phenomenonId: step.evidence.find((e) => e.type === 'measurement_recorded')
              ?.phenomenonId,
            payload: { t: command.t, value: command.value },
            discriminator: `m:${state.measurements.length}`,
          },
          ctx
        )
      );
    ...
    case 'parameter_changed':
      ...
      evidence.push(
        makeEvidence(
          {
            ...base,
            type: 'simulation_parameter_changed',
            payload: { parameter: command.parameter, value: command.value },
            discriminator: `p:${state.parameterChanges}`,
          },
          ctx
        )
      );
      break;
    case 'exercise': {
      const previous = state.answers?.[command.exercise.id] ?? {
        correct: false,
        score: 0,
        attempts: 0,
        value: undefined,
      ...
      const record = {
        correct: command.check.correct || previous.correct,
        score: Math.max(previous.score, command.check.score),
        attempts,
        value: command.value,
      };
      ...
      evidence.push(
        makeEvidence(
          {
            ...base,
            type: command.check.correct ? 'exercise_solved' : 'exercise_attempted',
            exerciseId: ex.id,
            nodeId: ex.nodeId,
            phenomenonId: ex.phenomenonId,
            dimension: ex.evidenceDimension,
            depth: ex.depth,
            result: command.check.correct
              ? 'correct'
              : command.check.score > 0
                ? 'partial'
                : 'incorrect',
            score: command.check.score,
            autonomy: autonomy(),
    ...
    case 'explanation': {
      ...
      state.answers = {
        ...
        [command.exerciseId]: {
          correct: true,
          score: 0.5,
          attempts: (state.answers?.[command.exerciseId]?.attempts ?? 0) + 1,
          value: command.text,
        },
      };
      evidence.push(
        makeEvidence(
          {
            ...
            nodeId: step.evidence.find((e) => e.type === 'explanation_submitted')?.nodeId,
        ...
        makeEvidence(
          {
            ...
            nodeId: command.toolId,
            phenomenonId: step.toolSelection.phenomenonId,
            dimension: 'modelling_choice',
            result: state.toolCorrect ? 'correct' : 'incorrect',
            autonomy: autonomy(),
            discriminator: `tool:${command.toolId}`,
          },
          ctx
        )
      );
    ...
    case 'open_hint':
      if (!state.hintsOpened.includes(command.hintId)) {
        ...
        evidence.push(
          makeEvidence(
            { ...base, type: 'hint_opened', discriminator: `hint:${command.hintId}` },
            ctx
          )
        );
      }
    ...
    case 'advance': {
      ...
      if (step.type === 'map_return' || nextStepId(steps, step, state) === null) {
        ...
        next.completedAt = now;
        completedMission = true;
        if (!step.evidence.some((e) => e.type === 'mission_completed'))
          evidence.push(
            makeEvidence({ ...base, type: 'mission_completed', discriminator: 'completed' }, ctx)
          );
      } else {
    ...
    case 'guide_repeat': {
      ...
      if (target)
        next.stepStates[command.stepId] = {
          ...target,
          status: 'active',
          answers: {},
          measurements: [],
        ...
        makeEvidence(
          {
            ...
            type: 'guide_rubric_scored',
            exerciseId: command.exerciseId,
            nodeId: command.nodeId,
            dimension: 'recognition_explanation',
            score: command.score,
            result: command.score >= 0.5 ? 'correct' : 'partial',
            autonomy: 1,
            depth: session.selectedDepth,
            discriminator: `rubric:${Math.round(command.score * 100)}`,
          },
          ctx
        )
      );
...
export function sessionProgress(
  ...
  return {
    index:
      Math.max(
        0,
        steps.findIndex((s) => s.id === session.currentStepId)
      ) + 1,
    total: steps.length,
  };
}
