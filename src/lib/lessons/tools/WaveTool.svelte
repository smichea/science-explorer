<script lang="ts">
  ...
  import { dopplerFrequency, waveDisplacement, waveQuantities } from '$lib/domain/lessonTools';
  ...
  const observerSpeed = $derived(
    tool.observerSpeed === undefined ? 0 : evaluateScalar(tool.observerSpeed, tstate.params)
  );
  const heard = $derived(dopplerFrequency(wave.frequency, wave.speed, sourceSpeed, observerSpeed));
  /** Width of the scene in metres: a dozen wavelengths, so the crowding is plain to see. */
  const sceneWidth = $derived(Math.max(1e-6, wave.wavelength * 12));
  const scene = $derived(
    scales(
      { x: [0, sceneWidth], y: [-sceneWidth / 4, sceneWidth / 4] },
      { W: 560, H: 280, pad: { l: 10, r: 10, t: 10, b: 24 } }
    )
  );
  /** How long the source is watched: the time it needs to cross most of the scene. */
  const sceneWindow = $derived(
    sourceSpeed !== 0 ? (0.7 * sceneWidth) / Math.abs(sourceSpeed) : wave.period * 8
  );
  const sceneTime = $derived(sceneWindow > 0 ? time % sceneWindow : 0);
  const sourceStart = $derived(sourceSpeed >= 0 ? 0.15 * sceneWidth : 0.85 * sceneWidth);
  const sourceX = $derived(sourceStart + sourceSpeed * sceneTime);
  const observerX = $derived(
    Math.max(0.02 * sceneWidth, 0.94 * sceneWidth - observerSpeed * sceneTime)
  );
  /** One circle per period elapsed, each centred where the source was when it left. */
  const fronts = $derived.by(() => {
    if (!doppler || wave.period <= 0) return [] as Array<{ x: number; r: number }>;
    const out: Array<{ x: number; r: number }> = [];
    for (let k = Math.floor(sceneTime / wave.period); k >= 0; k--) {
      const emitted = k * wave.period;
      const radius = wave.speed * (sceneTime - emitted);
      if (radius > 1.4 * sceneWidth) break;
      out.push({ x: sourceStart + sourceSpeed * emitted, r: radius });
    }
    return out;
  });
</script>
...
<div class="tool stack-sm" data-testid="wave-tool" data-playing={playing}>
  {#if doppler}
    <svg
      ...
      class="tool__svg"
      role="img"
      aria-label={t('lesson.wave.doppler')}
      data-testid="doppler-scene"
    >
      {#each fronts as front, i (i)}
        <circle
          cx={scene.sx(front.x)}
          cy={scene.sy(0)}
          r={Math.max(0, scene.sx(front.r) - scene.sx(0))}
          fill="none"
          stroke="#7f9cff"
          stroke-width="1.5"
          opacity="0.7"
    ...
    <p class="small" style="margin: 0" data-testid="wave-reading">
      ...
      {#if sourceSpeed !== 0}· {t(
          sourceSpeed > 0 ? 'lesson.wave.approaching' : 'lesson.wave.receding'
        )}{/if}
    </p>
  {:else}
    <svg
    ...
    >
      ...
      <text x={top.W - top.pad.r} y={top.H - 4} class="label" text-anchor="end"
        >{tool.labels.x}</text
      >
    ...
    >
      ...
      <text x={bottom.W - bottom.pad.r} y={bottom.H - 4} class="label" text-anchor="end"
        >{tool.labels.t}</text
      >
      <text x={bottom.pad.l + 4} y={bottom.pad.t - 2} class="label"
        >{t('lesson.wave.signal')} M</text
      >
